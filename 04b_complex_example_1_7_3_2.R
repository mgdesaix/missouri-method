...
# Specify the not main pop (i.e. not popB)
  ...
  popsOfInterest <- c("Pop1", "Pop2", "Pop7", "Pop3")
  sim_index <- which(Markers$ret_ids$group %in% popsOfInterest)

  ##############################################################################################################
  ###
  ### Part 1b: gscramble pedigree
  ###
  ##############################################################################################################

  ref_gscramble$I_meta <- Markers$ret_ids[sim_index,]
  ref_gscramble$Geno <- Markers$ret_geno[sim_index,]
  rownames(ref_gscramble$Geno) <- ref_gscramble$I_meta$indiv
  ...
  gsp_f1b1b2 <- create_GSP("p1", "p2", F1=TRUE, F1B=TRUE, F1B2=TRUE)

  gsp_f1b1b2$hpop1 <- c("p1", "p2", "p3", "p4", NA, NA, NA)
  gsp_f1b1b2$hpop2 <- c("p1", "p2", "p3", "p4", NA, NA, NA)
  
  RepPop <- tibble(
    index = as.integer(c(1,1,1,1)),
    pop = c("p1", "p2", "p3", "p4"),
    group = c("Pop1", "Pop3", "Pop2", "Pop7")
  )
  ...
  # Pull all simulated individuals
  outname <- paste0(outdir, "/complex_no_ghost.", iteration)
  ...
  Q_meta <- merged_fam[merged_index,] %>%
    ...
    mutate(id = as.numeric(str_split_i(Individual, "-", 4)),
           pedigree = ifelse(id == 5, "F1",
                                    ifelse(id == 6, "F1B1",
                                           ifelse(id == 7, "F1B2", NA)))) %>%
    cbind(Q_mat) %>% select(-c(id))

  # Remove intermediary files
  SysCommand <- paste0("rm ", outdir, "/complex_no_ghost.", iteration, ".*")
  system(SysCommand)

  summary_outname <- paste0(outdir, "/complex_no_ghost.", iteration, ".csv")
  write_csv(Q_meta, file = summary_outname)

} # End full iteration from Part 0

file_list <- paste0(outdir, "/complex_no_ghost.", start_iteration:final_iteration, ".csv")
file.df <- file_list %>% lapply(read_csv) %>% bind_rows()
summary_outname <- paste0(summary_outdir, "/complex_no_ghost.", start_iteration, "-", final_iteration, ".csv")
