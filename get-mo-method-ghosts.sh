#!/bin/bash
#set a job name
#SBATCH --job-name=supervised-admixture-ghost
#SBATCH --output=./err-out/supervised-admixture-ghost.%A_%a.out
#SBATCH --error=./err-out/supervised-admixture-ghost.%A_%a.err
...
start=$(awk -v N=$SLURM_ARRAY_TASK_ID 'NR == N {print $1}' ./data/admixture/ghost-start-end-array.txt)
final=$(awk -v N=$SLURM_ARRAY_TASK_ID 'NR == N {print $2}' ./data/admixture/ghost-start-end-array.txt)

genotypes=/lustrefs/nwrc/projects/DeSaix/MO-method/data/genotypes/MOMethod-SpatialPops1672x27866
matrix=/lustrefs/nwrc/projects/DeSaix/MO-method/data/genotypes/MOMethod-Ghosts3382x27866

# Final filename to store results
mo_final_matrix=/lustrefs/nwrc/projects/DeSaix/MO-method/out/admixture/empirical/Ghosts3382x27866-admixture-summary.txt

# Make tmp directory for intermediary files
tmpdir=/lustrefs/nwrc/projects/DeSaix/MO-method/out/admixture/tmp_Ghost_array_${SLURM_ARRAY_TASK_ID}
mkdir -p ${tmpdir}
cd ${tmpdir}
