...
outdir <- myargs[3]
ref_genos <- myargs[4]
ghost_genos <- myargs[5]
summary_outdir <- myargs[6]
...
# Specify the not main pop (i.e. not popB)
  ...
  outname <- paste0(outdir, "/complex_with_ghost.", iteration, ".scrambled")

  popsOfInterest <- c("Pop1", "Pop2", "Pop7")
  ...
  # Pull all simulated individuals
  outname <- paste0(outdir, "/complex_with_ghost.", iteration)
  ...
  SysCommand <- paste0("rm ", outdir, "/complex_with_ghost.", iteration, ".*")
  system(SysCommand)

  summary_outname <- paste0(outdir, "/complex_with_ghost.", iteration, ".csv")
  write_csv(Q_meta, file = summary_outname)

} # End full iteration from Part 0

file_list <- paste0(outdir, "/complex_with_ghost.", start_iteration:final_iteration, ".csv")
file.df <- file_list %>% lapply(read_csv) %>% bind_rows()
summary_outname <- paste0(summary_outdir, "/complex_with_ghost.", start_iteration, "-", final_iteration, ".csv")
