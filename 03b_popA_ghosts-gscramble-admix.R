...
# Specify the not main pop (i.e. not popB)
  ...
  outname <- paste0(outdir, "/ghost.", popB, ".", gsp_id, ".", iteration, ".scrambled")
  sim_index <- which(Markers$ret_ids$group == popB)
  ...
  # Pull all simulated individuals
  outname <- paste0(outdir, "/ghost.", popB, ".", gsp_id, ".", iteration)
  ...
  SysCommand <- paste0("rm ", outdir, "/ghost.", popB, ".", gsp_id, ".", iteration, ".*")
  system(SysCommand)

  summary_outname <- paste0(outdir, "/ghost.", popB, ".", gsp_id, ".", iteration, ".csv")
  write_csv(Q_meta, file = summary_outname)

} # End full iteration from Part 0

file_list <- paste0(outdir, "/ghost.", popB, ".", gsp_id, ".", start_iteration:final_iteration, ".csv")
file.df <- file_list %>% lapply(read_csv) %>% bind_rows()
summary_outname <- paste0(summary_outdir, "/ghost.", popB, ".", gsp_id, ".", start_iteration, "-", final_iteration, ".csv")
