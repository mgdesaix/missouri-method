#!/bin/bash
#set a job name
#SBATCH --job-name=04a_ghost-complex-admixture
#SBATCH --output=./err-out/04a_ghost-complex-admixture.%A_%a.out
#SBATCH --error=./err-out/04a_ghost-complex-admixture.%A_%a.err
...
summary_outdir=/lustrefs/nwrc/projects/DeSaix/MO-method/out/gscamble-workflow/summary/complex/ghosts
...
outdir=/lustrefs/nwrc/projects/DeSaix/MO-method/out/gscamble-workflow/complex/ghosts/tmp.1_7_ghost_2.${start_iteration}.${final_iteration}
mkdir -p ${outdir}

Rscript 04a_complex_example_1_7_ghost_2.R ${start_iteration} ${final_iteration} ${outdir} ${ref_genos} ${ghost_genos} ${summary_outdir}
