#!/bin/bash
#set a job name
#SBATCH --job-name=supervised-admixture-nonspat10
#SBATCH --output=./err-out/supervised-admixture-nonspat10.%A_%a.out
#SBATCH --error=./err-out/supervised-admixture-nonspat10.%A_%a.err
...
start=$(awk -v N=$SLURM_ARRAY_TASK_ID 'NR == N {print $1}' ./data/admixture/nonspat10-start-end-array.txt)
final=$(awk -v N=$SLURM_ARRAY_TASK_ID 'NR == N {print $2}' ./data/admixture/nonspat10-start-end-array.txt)

genotypes=/lustrefs/nwrc/projects/DeSaix/MO-method/data/genotypes/MOMethod-SpatialPops1672x27866
matrix=/lustrefs/nwrc/projects/DeSaix/MO-method/data/genotypes/MOMethod-NonSpat10x27866

# Final filename to store results
mo_final_matrix=/lustrefs/nwrc/projects/DeSaix/MO-method/out/admixture/empirical/NonSpat10x27866-admixture-summary.txt

# Make tmp directory for intermediary files
tmpdir=/lustrefs/nwrc/projects/DeSaix/MO-method/out/admixture/tmp_NonSpat10_array_${SLURM_ARRAY_TASK_ID}
mkdir -p ${tmpdir}
cd ${tmpdir}
